...
fn simulate_bingo(values: Vec<i16>, mut boards: Vec<Vec<Vec<i16>>>) -> i64 {
  ...
  let mut last_board = boards.clone();
  for value in values {
    last_value = value;
    x_value(value, &mut boards);
    let mut non_bingo_boards = Vec::new();
    for board in boards.iter() {
      if !check_bingo(board) {
        non_bingo_boards.push(board.clone());
      }
    }
    last_board = boards.clone();
    boards = non_bingo_boards;
    if boards.len() == 0 {
      break;
    }
  }
  get_free_values(&last_board[0]).iter().sum::<i16>() as i64 * last_value as i64
}
