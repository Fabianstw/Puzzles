...
fn draw_lines_all(coordinate_lines: Vec<(i32, i32, i32, i32)>) -> i32 {
  ...
  for line in coordinate_lines.iter() {
    ...
    } else if (line.0 - line.2).abs() == (line.1 - line.3).abs() {
      for add_value in 0..max(line.0, line.2) - min(line.0, line.2) + 1 {
        let x = line.0
                + if line.0 < line.2 {
                  add_value
                } else {
...
pub fn solve() {
  ...
  let res = draw_lines_all(coordinate_lines);
